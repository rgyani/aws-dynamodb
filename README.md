...
Amazon DynamoDB is a semi-structured wide column store. It can be used both as either a key-value store or you can build a structured schema.
...
| GET_author_by_name | GET_all_author_ingormation | GET_books_by_author |
| --- |--- |---|
|response = table.get_item(<br>&emsp;Key={<br>&emsp;&emsp;"PK": f"AUTHOR#{author_name}",<br>&emsp;&emsp;"SK": "METADATA"<br>&emsp;}<br>)| response = table.query(<br>&emsp;KeyConditionExpression=conditions.Key("PK").eq(f"AUTHOR#{author_name}")<br>)| response = table.query(<br>&emsp;KeyConditionExpression=conditions.Key("PK").eq(f"AUTHOR#{author_name}") & conditions.Key("SK").begins_with("ISBN")<br>)|
...
1 (one) RCUs is 1 item maximum of max 4 (four) Kb size in a strong consistency or two items in an eventual consistency (we’ll cover DynamoDB consistency in the next chapter) of max 4 Kb s.
